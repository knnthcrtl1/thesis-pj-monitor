<?php
    ...
    if (isset($_POST['ajax'])) {
        ...
        if ($_POST["function-type"] === "add-project") {
            ...
            $projectTableFields = "project_name,project_contractor_name,project_telephone,project_work_location,project_issuing_office,project_issuing_addres,project_start_date,project_end_date,project_client_owner";
            $sql = "INSERT INTO tbl_projects ( {$projectTableFields} ) VALUES 
                ('{$projectName}','{$projectContractorName}','{$projectTelephone}','{$projectWorkLocation}','{$projectIssuingOffice}','{$projectIssuingAddress}','{$projectStartDate}','{$projectEndDate}','{$projectClientOwner}')";
